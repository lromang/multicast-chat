...
public class CausalHandler extends Thread {
	...
	public CausalHandler(String groupIp, int port, int ttl, int repPortServer, int repPortClient) throws IOException {
	...
	public void run() {
		...
		while(running) {
				...
				receivedMessage = MessageHandler.getMessageFrom(datagramPacket.getData());
							...
							if(!process.equals(processId)) {
								...
								if(seen<=sent-1) {
									...
									for(int messageId=seen+1; messageId<sent; messageId++) {
										requestedMessage = requestMessage(commandListener.getIpFromProcessId(process), 
												messageId, Config.MAX_BUFFER_DIM);
										commandListener.receivedMessage(requestedMessage.getPayload());
										Logger.println("Pulled message " + requestedMessage.toString());
									}
									// push first received message
									if(process.equals(receivedMessage.getProcessId())) {
										commandListener.receivedMessage(receivedMessage.getPayload());
									}
								}
							}
						}
				...
				Logger.println("UnknownHostException in CausalHandler Thread: " + e.getMessage());
			} catch (IOException e) {
